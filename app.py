...
def main():
    ...
    for row in rows:
        if(row[69] == 'GB'):
            ...
            bankinfo = bank_details('GB', row[93], row[90])
            ...
            data = [
                    ...
                    ['A', 'AD1', '', row[60], row[61], row[62], row[67], row[65], 'GB', row[118], row[119], row[115]],
                    ['R', 'GB', bankinfo['iban'], bankinfo['bic']],
            ]
            # Open writer for output
            csv.writer(open('SuppliersGB.csv', 'a', newline='')).writerows(data)
